...
resource "oci_identity_compartment" "newrelic_compartment" {
  ...
  name           = "newrelic_compartment_ORM_DO_NOT_REMOVE_${local.random_id}"
...
resource "oci_vault_secret" "ingest_api_key" {
  ...
  secret_name    = "newrelic_ingest_api_key_ORM_DO_NOT_REMOVE_${local.random_id}"
  secret_content {
    content_type = "BASE64"
...
resource "oci_vault_secret" "user_api_key" {
  ...
  vault_id       = oci_kms_vault.newrelic_vault[count.index].id
  key_id         = oci_kms_key.newrelic_key[count.index].id
  secret_name    = "newrelic_user_api_key_ORM_DO_NOT_REMOVE_${local.random_id}"
  secret_content {
    content_type = "BASE64"
